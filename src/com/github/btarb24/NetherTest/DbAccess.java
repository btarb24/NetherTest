...
{	
	...
	public void initDbConnection()
	{
		try {
			...
			{
				_connection = DriverManager.getConnection (_config.getProperty(Keys.DB_URL));
				_statement = null;
			}
	...
	public boolean canEnter(Player player) throws SQLException
	{
		...
		cal.add(Calendar.HOUR, _config.getPropertyInt(Keys.ENTRANCE_FREQUENCY));
		...
		{//the user gets to start a completely new set of sessions
			...
			rs.updateInt(_config.getProperty(Keys.DB_FIELD_MINS), 0);
		...
		else
		{
			...
			minutes = rs.getInt(_config.getProperty(Keys.DB_FIELD_MINS));
			
			//they've used too many minutes.. throw up an error and fail out
			if (minutes > _config.getPropertyInt(Keys.MAX_SESSION_LENGTH))
			{
				...
				player.sendMessage(String.format("You've exceeded the %s minute maximum per %s hours", _config.getProperty(Keys.MAX_SESSION_LENGTH), _config.getProperty(Keys.ENTRANCE_FREQUENCY)));
		...
		player.sendMessage(String.format("Welcome to the Nether World. You have %d minutes left.", _config.getPropertyInt(Keys.MAX_SESSION_LENGTH) - minutes));
	...
	public boolean surpassedLimit (Player player) throws SQLException
	{
		...
		int previousMinutes = rs.getInt(_config.getProperty(Keys.DB_FIELD_MINS));
		
		//add it and save it to the db
		int totalMinutes = minutesSpent + previousMinutes;
		
		//did they exceed?
		return totalMinutes > _config.getPropertyInt(Keys.MAX_SESSION_LENGTH);
	}
	...
	{		
		...
		try {
			...
			//add it and save it to the db
			int totalMinutes = minutesSpent + previousMinutes;
			rs.updateInt(_config.getProperty(Keys.DB_FIELD_MINS), totalMinutes);
			...
			player.sendMessage(String.format("You just spent %d minutes in Nether and a total of %d. The limit is %s.", minutesSpent, totalMinutes, _config.getProperty(Keys.MAX_SESSION_LENGTH)));
	...
	public void outputInfo(CommandSender sender, String playername)
	{
		...
		try {
			...
			if (player != null && player.getWorld().getName().equals(_config.getProperty(Keys.NETHER_SERVER_NAME)))
			{
			...
			{	
				...
				sender.sendMessage(String.format(
						...
						_config.getProperty(Keys.MAX_SESSION_LENGTH),
						_config.getProperty(Keys.ENTRANCE_FREQUENCY),
						calcHour,
						calcMin));
			}
			else //all minute available.
			{
				sender.sendMessage(String.format("%s has all %s minutes available.", playername,  _config.getProperty(Keys.MAX_SESSION_LENGTH)));;
			}
		} 
		catch (SQLException e) {
	...
	{//Player needs their session minutes persisted and timestamp updated.
		...
		try {
			...
			//max minutes
			rs.updateInt(_config.getProperty(Keys.DB_FIELD_MINS), _config.getPropertyInt(Keys.MAX_SESSION_LENGTH));
	...
	private ResultSet retrieveRecord(String playername) throws SQLException
	{
		...
		{//they don't have a record yet.. make one so we can return it
			...
			rs.updateString(_config.getProperty(Keys.DB_FIELD_NAME), playername); //make a record (other fields are auto-pop)
	...
	private Calendar getTimestampFromDb(ResultSet rs) throws SQLException
	{
		...
		//get the val from db
		Timestamp dbTime = rs.getTimestamp(_config.getProperty(Keys.DB_FIELD_TIME), cal);
	...
	private void updateTimestamp(ResultSet rs) throws SQLException
	{
		...
		rs.updateTimestamp(_config.getProperty(Keys.DB_FIELD_TIME), ts);
	}
